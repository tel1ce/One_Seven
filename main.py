...
add_read_markup = tbt.types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
        ...
        if int(call.data) in range(32):
            if role == "user":
                bot.send_message(call.message.chat.id,
                                 text=f"Выбрана дата {d}.{m}.{YEAR}. Отправляем информацию:",# Все работает но может есть смысл сменить на date
                                 reply_markup=add_read_markup)
